...
struct Deck {
    ...
    func drawOne() -> Card? {
        if deck.count > 0 {
            let size = deck.count
            let randomInt = Int((arc4random_uniform(UInt32(size))))
...
class Play {
    ...
    func whoIsWinner(){
        playerCard = deck.drawOne()
        computerCard = deck.drawOne()
        
        if let playerCard = playerCard{
            if let computerCard = computerCard{
                if  playerCard.rank.rawValue >  computerCard.rank.rawValue{
                    self.printMessage("PLAYER", card: playerCard)
                }else{
                    self.printMessage("Computer", card: computerCard)                }
            } else{
                print("Computer doesnt have a card")
            }
        } else{
            print("Player doeusn't have a card. Out of cards in deck")
        }
        
    }
    
    
    func printMessage(winner: String, card: Card){
        
        print("The \(winner) won with the \(card.rank) of \(card.suit)!")
    }
